---
world_id: attention_free_llm_research
name: Attention-Free LLM Architectures Research
version: 1.0.0
...
Autonomous AI research loops investigating attention-free LLM architectures must operate within structured governance: experiments are reproducible, metrics are tracked, compute budgets are enforced, and agents cannot drift beyond their declared research context. A research world without constraints produces noise, not knowledge.
...
- `dataset_must_be_declared` — The dataset "TinyStories" must be used for training and evaluation and never changed without governance approval (structural, immutable)
...
- `no_softmax_attention` — Architectures must not use softmax attention mechanisms; this is the core constraint of the research context (prompt, immutable)
...
- description: Best value achieved for val_bpb
