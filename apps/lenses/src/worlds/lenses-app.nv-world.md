...
- `no_behavioral_persistence` — Lens directives shape the current session only. The app does not build a behavioral profile of the user across sessions. Each session starts fresh. (structural, immutable)
...
- `ambient_user_initiated_only` — Ambient context is only included in an AI call when the user explicitly triggers activation (tap, double-tap, wake word). The buffer is passive — it listens but never acts on its own. (structural, immutable)
