...
- `no_cross_tool_data_leak` — Documents uploaded to the Document Hub are classified and routed to specific tools. The user can see EXACTLY which tools can access which documents. No silent cross-tool access. If Audit needs a doc that was uploaded for Replay, the user sees a prompt: "This document is currently available to Replay. Also make it available to Audit?" Transparency, not restriction. (structural, immutable)
