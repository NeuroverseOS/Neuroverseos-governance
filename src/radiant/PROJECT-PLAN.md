...
- **`neuroverseos-sovereign-conduit.worldmodel.md`** lives at [github.com/NeuroverseOS/Worlds](https://github.com/NeuroverseOS/Worlds) — the canonical source of truth. Every NeuroverseOS repo auto-inherits it via the org-detect discovery tier (zero per-repo config). A reference sample sits at `src/radiant/examples/neuroverse-base/` for developers who want to see the format.
...
15. ✅ **MCP server entry** — `src/radiant/mcp/server.ts`. JSON-RPC 2.0 over stdio. Exposes `radiant_think` and `radiant_emergent` as MCP tools. Any MCP-compatible client (Claude Code, Cursor) can call them in conversation. CLI: `neuroverse radiant mcp --worlds ./worlds/ --lens auki-builder`. Config snippet for Claude Code's `.claude/settings.json` included in Auki README.
